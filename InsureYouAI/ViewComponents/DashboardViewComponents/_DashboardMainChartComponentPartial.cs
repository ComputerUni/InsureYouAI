...
{
    public class _DashboardMainChartComponentPartial(InsureContext _context) : ViewComponent
    {
        public IViewComponentResult Invoke()
        {
            //Revenue
            var revenueData = _context.Revenues.GroupBy(r => r.ProcessDate.Month).Select(g => new
            {
                Month = g.Key,
            ...
            //Expense
            var expenseData = _context.Expenses.GroupBy(e => e.ProcessDate.Month).Select(g => new
            {
                Month = g.Key,
                TotalAmount = g.Sum(x => x.Amount)
            }).OrderBy(x => x.Month).ToList();
            ...
            {
                Months = allMonths.Select(x => new DateTimeFormatInfo().GetAbbreviatedMonthName(x)).ToList(),
